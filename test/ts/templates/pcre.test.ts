...
describe('matchPCRE', () => {
    ...
    testCases.forEach(testCase => {
        test(testCase.name, () => {
            ...
            expect({ ...result, match: !!result.match }).to.deep.equal(testCase.expected);
        });
    });
});
...
describe('applyPCREMatches', () => {
    ...
    testCases.forEach(testCase => {
        test(testCase.name, () => {
            ...
            expect({ captureGroupKeys, match: !!match }).to.deep.equal(expected);
            expect(result).to.deep.equal(expectedNewDest);
        });
    });
});
